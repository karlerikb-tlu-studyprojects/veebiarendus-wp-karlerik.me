...
<div id="reviews" class="woocommerce-Reviews">
	<div id="comments">
		<h2 class="woocommerce-Reviews-title"><?php
			if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' && ( $count = $product->get_review_count() ) ) {
				...
				printf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'square' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span>' );
			} else {
				_e( 'Reviews', 'square' );
			}
		?></h2>
		...
		<?php else : ?>
			...
			<p class="woocommerce-noreviews"><?php _e( 'There are no reviews yet.', 'square' ); ?></p>
		...
		<div id="review_form_wrapper">
			<div id="review_form">
				<?php
					...
					$comment_form = array(
						'title_reply'          => have_comments() ? __( 'Add a review', 'square' ) : sprintf( __( 'Be the first to review &ldquo;%s&rdquo;', 'square' ), get_the_title() ),
						'title_reply_to'       => __( 'Leave a Reply to %s', 'square' ),
						...
						'label_submit'  => __( 'Submit', 'square' ),
						'logged_in_as'  => '',
						'comment_field' => '',
					);

					if ( $account_page_url = wc_get_page_permalink( 'myaccount' ) ) {
						$comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a review.', 'square' ), esc_url( $account_page_url ) ) . '</p>';
					}
					...
					if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
						$comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__( 'Your rating', 'square' ) . '</label><select name="rating" id="rating" aria-required="true" required>
							...
							<option value="3">' . esc_html__( 'Average', 'square' ) . '</option>
							<option value="2">' . esc_html__( 'Not that bad', 'square' ) . '</option>
							<option value="1">' . esc_html__( 'Very poor', 'square' ) . '</option>
						</select></div>';
					}
	...
	<?php else : ?>
		...
		<p class="woocommerce-verification-required"><?php _e( 'Only logged in customers who have purchased this product may leave a review.', 'square' ); ?></p>
